"""
passkeys.py
100 unique multi-use activation passkeys for Ember.

Each key is branded with the EMBR- prefix followed by a 4-character
alphanumeric segment. Keys are case-insensitive on validation.
...
PASSKEYS: frozenset[str] = frozenset({
    # ── Block 1 ──────────────────────────────────────────────────────
...
def is_valid(code: str) -> bool:
    """Check whether *code* is a recognised passkey (case-insensitive)."""
    if not code or not isinstance(code, str):
        return False
    return code.strip().upper() in PASSKEYS
